...
import ast
with open('quantum_engine/optimizer.py') as f:
    src = f.read()
ast.parse(src)
print("Written and syntax verified!")
...
sys.path.insert(0, '.')
import importlib
import quantum_engine.optimizer as m
importlib.reload(m)
opt = m.QuantumIngredientsOptimizer('data/ingredients.json')
result = opt.get_optimized_ingredients('Italian', None, False, 'pasta', 8)
print("Test OK! Selected:", result['selected_ingredients'])